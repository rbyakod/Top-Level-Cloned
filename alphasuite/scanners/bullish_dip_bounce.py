...
class BullishDipBounceScanner(BaseScanner):
    """
    Scans for a "buy the dip" opportunity within a primary uptrend.

    Goal:
        Identify a high-probability trend-continuation entry. This pattern occurs
        when a stock in an uptrend has a pullback that makes a lower price low,
        but the RSI fails to confirm this with a lower low, suggesting the
        pullback is losing steam and the primary uptrend will resume.

    Criteria:
        - The stock is in a general uptrend (price > 200-day SMA).
        - The price has made a lower low compared to a recent trough.
        - The RSI has made a higher low compared to the RSI at the previous price trough.
    """
    @staticmethod
    ...
    def scan_company(self, group: pd.DataFrame, company_info: dict) -> dict | None:
        ...
        for i in range(1, setup_lookback_days + 1):
            ...
            is_lower_low_price = group['low'].iloc[-i] < lookback_window.loc[min_price_day_idx, 'low']
